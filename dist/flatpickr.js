    ...
    ***************************************************************************** */
                ...
                if (datestr === "today") {
                    ...
                    for (var i = 0, matchIndex = 0, regexStr = ""; i < format.length; i++) {
                        ...
                        if (config.parseTokenRegexs[token] && !escaped) {
                            var parseTokenRegex = config.parseTokenRegexs[token];
                    ...
                    ops.sort(function (a, b) { return a.order - b.order; })
                        .forEach(function (_a) {
    ...
    function FlatpickrInstance(element, instanceConfig) {
        ...
        function setupHelperFunctions() {
            self.utils = {
                getDaysInMonth: function (monthIndex, centuryYear) {
                    if (monthIndex === void 0) { monthIndex = self.currentMonth; }
                    if (centuryYear === void 0) { centuryYear = self.currentYear; }
        ...
        function setHoursFromInputs() {
            ...
            var hours = (parseInt(self.hourElement.value.slice(-2), 10) || 0) % 24, minutes = (parseInt(self.minuteElement.value, 10) || 0) % 60, seconds = self.secondElement !== undefined
                ...
                : 0, milliseconds = self.millisecondElement
        ...
        function setHours(hours, minutes, seconds, milliseconds) {
            ...
            if (self.latestSelectedDateObj !== undefined) {
                self.latestSelectedDateObj.setHours(hours % 24, minutes, seconds, milliseconds);
            }
        ...
        function buildMonthDays(year, month) {
            ...
            var prevMonthDays = getDaysInMonth((month - 1 + 12) % 12, year, self.l10n);
            var daysInMonth = getDaysInMonth(month, year, self.l10n), days = window.document.createDocumentFragment(), isMultiMonth = self.config.showMonths > 1, prevMonthDayClass = isMultiMonth ? "prevMonthDay hidden" : "prevMonthDay", nextMonthDayClass = isMultiMonth ? "nextMonthDay hidden" : "nextMonthDay";
        ...
        function parseConfig() {
            ...
            HOOKS.forEach(function (hookName) {
                userConfig[hookName] = self.config[hookName].concat(arrayify(userConfig[hookName] || []).map(bindToInstance));
            });
        ...
        function setDate(date, triggerChange, format, doSkipUpdateInputElement) {
            if (triggerChange === void 0) { triggerChange = false; }
            if (format === void 0) { format = self.config.dateFormat; }
            if (doSkipUpdateInputElement === void 0) { doSkipUpdateInputElement = false; }
        ...
        function timeWrapper(e) {
            ...
            var min = parseFloat(input.getAttribute("min")), max = parseFloat(input.getAttribute("max")), step = parseFloat(input.getAttribute("step")), digits = max.toString().length, curValue = parseInt(input.value, 10), delta = e.delta ||
    ...
    flatpickr.getStaticConfig = function () {
        return __assign({}, defaults, flatpickr.defaultConfig);
    };
    flatpickr.l10ns = {
        en: __assign({}, english),
        "default": __assign({}, english)
    };
    var rebuildFormatParse = function () {
        var globalConfig = flatpickr.getStaticConfig();
